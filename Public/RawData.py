...
RawData_Message_Names=  (
   'Real-time GPS survey data (RT-17)',
   'Position Record (RT-11)',
   'Event Mark',
   'MET3 (Internal)',
   'SBAS Raw (Internal)',
...
class GSOF (DCOL.Dcol) :
    def __init__ (self):
        self.last_page=-2
        self.max_page=-1
        self.RawData_Buffer=bytearray([])
        self.seq_number=-1
        self.seen_pages=set([])
        self.subrecord=-1

    def decode_page_number(byte):
        # Mask and shift to get the current page number (bits 4-7)
        current_page_number = (byte & 0xF0) >> 4
        # Mask to get the page total (bits 0-3)
        page_total = byte & 0x0F
        return current_page_number, page_total


    def decode(self,data,internal=False):
        unpacked=unpack_from('> B B B B',data)
#        print " Buffer {:02X} {}".format(len(self.RawData_Buffer),len(self.RawData_Buffer))
#        print " GSOF: " + hexlify(self.RawData_Buffer)
        del data[0:3]
#        print " PACKET: " + str(len(data)) + " " +hexlify(data)
        self.subrecord=unpacked[0]

        current_page, total_pages = decode_page_number(unpacked[1])
...
#                print " Buffer Final Buffer {}".format(len(self.RawData_Buffer))
#                print hexlify(self.RawData_Buffer)
                while self.RawData_Buffer :
                    unpacked=unpack_from('> B B',self.RawData_Buffer)
                    subrecord=unpacked[0];
                    self.seen_subrecords|=set([subrecord])
                    length=unpacked[1]

#                    print "sub: {} Length: {:02X}".format (unpacked[0],unpacked[1]);
#                    print "GSOF Buffer Length: " + str(len(self.RawData_Buffer))
#                    print hexlify(self.RawData_Buffer)

                    del self.RawData_Buffer [0:2]
 #                   print "GSOF Buffer Length after delete length: " + str(len(self.RawData_Buffer))

                    if subrecord == GSOF_POSITION_TIME :
                        unpacked=unpack_from('>L H B B B B',self.RawData_Buffer)
                        ...
                        unpacked=unpack_from('>d d d',self.RawData_Buffer)
                        self.Lat=unpacked[0]
                        ...
                        unpacked=unpack_from('> 8S d d d',self.RawData_Buffer)
                        ...
                        unpacked=unpack_from('> 8s 8s d d d',self.RawData_Buffer)
                        self.Datum_ID=unpacked[0]
                        ...
                        if length == 0x11 :
                           unpacked=unpack_from('>B f f f f',self.RawData_Buffer)
                           ...
                           unpacked=unpack_from('>B f f f',self.RawData_Buffer)
                           self.Velocity_Flags=unpacked[0]
                           self.Velocity=unpacked[1]
                           self.Heading=unpacked[2]
                           self.Vertical_Velocity=unpacked[3]
                        ...
                        unpacked=unpack_from('>f f f f',self.RawData_Buffer)
                        ...
                        unpacked=unpack_from('>B d d',self.RawData_Buffer)
                        ...
                        unpacked=unpack_from('>f f f f f f f f H',self.RawData_Buffer)
                        ...
                        unpacked=unpack_from('>f f f f f f f f f  H',self.RawData_Buffer)
...
#                        print "GSOF Buffer Length Brief start : " + str(len(self.RawData_Buffer))
                        SV_Brief_Buffer=bytearray()
                        SV_Brief_Buffer[:]=self.RawData_Buffer #Force to get a copy not a reference
...
#                                print "  {:02X}".format(len(SV_Brief_Buffer))
#                                print hexlify(SV_Brief_Buffer)
#                        print "GSOF Buffer Length brief end: " + str(len(self.RawData_Buffer))
...
#                        print "Start of decode"
                        ...
                        SV_Detail_Buffer[:]=self.RawData_Buffer #Force to get a copy not a reference
...
#                                print unpacked
                        ...
                        unpacked=unpack_from('>L',self.RawData_Buffer)
                        self.Serial_Number = unpacked[0]

                    elif subrecord == GSOF_CURRENT_TIME_INFORMATION :
                        unpacked=unpack_from('>L H H B',self.RawData_Buffer)
                        ...
                        unpacked=unpack_from('>L H B B B',self.RawData_Buffer)
                        ...
                        unpacked=unpack_from('>L B B B B d d d d H f f f f',self.RawData_Buffer)
...
#                        print "Start of decode"
                        SV_Detail_Buffer=bytearray()
                        SV_Detail_Buffer[:]=self.RawData_Buffer
...
#                                print unpacked
                        ...
                        unpacked=unpack_from('>B 8s H d d d',self.RawData_Buffer)
                        ...
                        unpacked=unpack_from('>H d',self.RawData_Buffer)
                        self.Battery_Capacity=unpacked[0]
                        self.Memory_Left=unpacked[1]

                    elif subrecord == GSOF_RtkErrorScale:
                        unpacked=unpack_from('>f B B f B',self.RawData_Buffer)
                        ...
                        unpacked=unpack_from('>5s f H f B B B B B f f B f f L L L L L L B d',self.RawData_Buffer)
                        ...
                        unpacked=unpack_from('>L H d d d B',self.RawData_Buffer)
...
#                        print len(self.RawData_Buffer)
#                        print hexlify(self.RawData_Buffer)
                        del self.RawData_Buffer[0:length]
#                        print "After Remove"
#                        print len(self.RawData_Buffer)
#                        print hexlify(self.RawData_Buffer)
...
#                print " Subrecord: {}".format(subrecord);
                ...
                if Dump_Level >= Dump_Summary:
                    ...
                    if subrecord == GSOF_POSITION_VCV_INFO :
                        unpacked=unpack_from('>f f f f f f f f H',str(self.RawData_Buffer))
